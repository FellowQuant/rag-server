...
- [x] **Phase 7: Pip Package & Claude Code Auto-Config** - `pip install fellowquant-rag` with auto-configured MCP server (completed 2026-02-21)
...
### Phase 7: Pip Package & Claude Code Auto-Config
**Goal**: `pip install fellowquant-rag` installs the full stack and auto-configures the MCP server in Claude Code with user choice of global or per-project scope
**Depends on**: Phase 6
**Requirements**: PKG-01, PKG-02, PKG-03, PKG-04
**Success Criteria** (what must be TRUE):
  1. User runs `pip install fellowquant-rag` and the package is installed with all CLI entry points
  2. Post-install setup runs automatically and prompts user to register MCP globally or locally
  3. Global registration: `claude mcp add --scope user` with `rag-server mcp` entry point + DATA_DIR prompt
  4. Local registration: `.mcp.json` written in CWD; fallback when `claude` CLI is absent
  5. `rag-server setup` is idempotent — re-running detects existing config and prompts update/skip
  6. `rag-server start`, `rag-server mcp`, `rag-server start-qdrant` CLI commands all work
  7. `llm.yaml.example` is copied to CWD on first setup; DATA_DIR defaults to `./data/`
**Plans**: 3 plans

Plans:
- [ ] 07-01-PLAN.md — Package structure: pyproject.toml entry points, hatch build, embedded assets (docker-compose.yml, llm.yaml.example)
- [ ] 07-02-PLAN.md — CLI commands: rag-server start / mcp / start-qdrant + setup wizard (scope prompt, MCP registration, config copy, idempotency)
- [ ] 07-03-PLAN.md — PyPI publish workflow (GitHub Actions + Trusted Publisher OIDC) + README install section + smoke test
...
| 7. Pip Package & Claude Code Auto-Config | 2/3 | Complete    | 2026-02-21 |
