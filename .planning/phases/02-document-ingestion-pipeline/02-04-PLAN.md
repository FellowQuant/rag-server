...
async def upload_document(
    ...
    DATA_DIR/uploads/, creates Document record (status=pending), enqueues
    IngestionJob, and returns immediately with 202.
    ...
    db.add(doc)
    await db.flush()   # assigns created_at/updated_at via defaults
...
async def lifespan(app: FastAPI):
    ...
    Startup:
      ...
      4. Start ingestion worker process (loads BGE-M3)
...
# Verify main app import (does NOT start server)
# Note: set_start_method may raise on re-import — that's OK in test context
import importlib.util
spec = importlib.util.spec_from_file_location('main_check', 'src/rag_server/main.py')
# Just verify schemas and router import cleanly
print('All imports OK')
"
```
  </verify>
  <done>DocumentUploadResponse, DocumentStatusResponse, DocumentListItem, DocumentListResponse all instantiate with correct fields. Router has routes for POST /documents, GET /documents, GET /documents/{id}, DELETE /documents/{id}. main.py imports without error. config.py ensure_data_dirs() creates uploads/ subdirectory.</done>
</task>
